...
import { adminGetSong } from "@/lib/admin/api";
import { updateSongAction } from "@/lib/admin/actions";
...
export default async function EditSongPage({ params }: Props) {
  ...
  const song = await adminGetSong(slug);
  if (!song) notFound();

  const action = updateSongAction.bind(null, slug);
  ...
  return (
    <div className="min-h-screen bg-cp-bg">
      <header className="bg-cp-surface border-b border-cp-border px-6 py-4 flex items-center gap-4">
        ...
        <h1 className="text-sm font-semibold text-cp-text truncate">Edit — {song.title}</h1>
      </header>
      <main className="px-6 py-8 max-w-3xl mx-auto">
        <SongForm song={song} action={action} submitLabel="Save Changes" />
      </main>
    </div>
  );
}
